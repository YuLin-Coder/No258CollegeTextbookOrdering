			...
			<el-col v-if="user_group === '管理员' || $check_field('get','textbook_name') || $check_field('add','textbook_name') || $check_field('set','textbook_name')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="教材名称" prop="textbook_name">
					<el-input id="textbook_name" v-model="form['textbook_name']" placeholder="请输入教材名称"
							  v-if="user_group === '管理员' || (form['book_shortage_information_id'] && $check_field('set','textbook_name')) || (!form['book_shortage_information_id'] && $check_field('add','textbook_name'))" :disabled="disabledObj['textbook_name_isDisabled']"></el-input>
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','textbook_type') || $check_field('add','textbook_type') || $check_field('set','textbook_type')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="教材类型" prop="textbook_type">
					<el-input id="textbook_type" v-model="form['textbook_type']" placeholder="请输入教材类型"
							  v-if="user_group === '管理员' || (form['book_shortage_information_id'] && $check_field('set','textbook_type')) || (!form['book_shortage_information_id'] && $check_field('add','textbook_type'))" :disabled="disabledObj['textbook_type_isDisabled']"></el-input>
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','textbook_subjects') || $check_field('add','textbook_subjects') || $check_field('set','textbook_subjects')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="教材科目" prop="textbook_subjects">
					<el-input id="textbook_subjects" v-model="form['textbook_subjects']" placeholder="请输入教材科目"
							  v-if="user_group === '管理员' || (form['book_shortage_information_id'] && $check_field('set','textbook_subjects')) || (!form['book_shortage_information_id'] && $check_field('add','textbook_subjects'))" :disabled="disabledObj['textbook_subjects_isDisabled']"></el-input>
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','purchasing_personnel') || $check_field('add','purchasing_personnel') || $check_field('set','purchasing_personnel')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="采购人员" prop="purchasing_personnel">
						<div v-if="user_group !== '管理员'">
							...
							<!--v-if="user_group === '管理员' || (form['book_shortage_information_id'] && $check_field('set','purchasing_personnel')) || (!form['book_shortage_information_id'] && $check_field('add','purchasing_personnel'))" :disabled="disabledObj['purchasing_personnel_isDisabled']"></el-input>-->
							<!--<div v-else-if="$check_field('get','purchasing_personnel')">{{form['purchasing_personnel']}}</div>-->
							<el-select v-if="user_group === '管理员' || (form['book_shortage_information_id'] && $check_field('set','purchasing_personnel')) || (!form['book_shortage_information_id'] && $check_field('add','purchasing_personnel'))" id="purchasing_personnel" v-model="form['purchasing_personnel']" :disabled="disabledObj['purchasing_personnel_isDisabled']">
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','number_of_books') || $check_field('add','number_of_books') || $check_field('set','number_of_books')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="书籍数量" prop="number_of_books">
					<el-input id="number_of_books" v-model="form['number_of_books']" placeholder="请输入书籍数量"
							  v-if="user_group === '管理员' || (form['book_shortage_information_id'] && $check_field('set','number_of_books')) || (!form['book_shortage_information_id'] && $check_field('add','number_of_books'))" :disabled="disabledObj['number_of_books_isDisabled']"></el-input>
					<div v-else-if="$check_field('get','number_of_books')">{{form['number_of_books']}}</div>
				</el-form-item>
			</el-col>
			<el-col v-if="user_group === '管理员' || $check_field('get','record_date') || $check_field('add','record_date') || $check_field('set','record_date')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="记录日期" prop="record_date">
					<el-date-picker :disabled="disabledObj['record_date_isDisabled']" v-if="user_group === '管理员' || (form['book_shortage_information_id'] && $check_field('set','record_date')) || (!form['book_shortage_information_id'] && $check_field('add','record_date'))" id="record_date"
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','remarks') || $check_field('add','remarks') || $check_field('set','remarks')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="备注" prop="remarks">
					<el-input id="remarks" v-model="form['remarks']" placeholder="请输入备注"
							  v-if="user_group === '管理员' || (form['book_shortage_information_id'] && $check_field('set','remarks')) || (!form['book_shortage_information_id'] && $check_field('add','remarks'))" :disabled="disabledObj['remarks_isDisabled']"></el-input>
...
<script>
	...
	export default {
		...
		data() {
			return {
				field: "book_shortage_information_id",
				url_add: "~/api/book_shortage_information/add?",
				url_set: "~/api/book_shortage_information/set?",
				url_get_obj: "~/api/book_shortage_information/get_obj?",
				url_upload: "~/api/book_shortage_information/upload?",

				query: {
					"book_shortage_information_id": 0,
				},
				...
				form: {
					...
					"record_date":'', // 记录日期
					"remarks":'', // 备注
					"book_shortage_information_id": 0, // ID
        ...
        if (this.form["record_date"].indexOf("-")===-1){
        ...
        if(this.form["record_date"]=="0000-00-00"){
          this.form["record_date"] = null;
			...
			is_view(){
				...
				if(!bl){
					bl = this.$check_action('/book_shortage_information/table','add');
					console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
				}
				if(!bl){
					bl = this.$check_action('/book_shortage_information/table','set');
					console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
				}
				if(!bl){
					bl = this.$check_action('/book_shortage_information/view','add');
					console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
				}
				if(!bl){
					bl = this.$check_action('/book_shortage_information/view','set');
					console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
				}
				if(!bl){
					bl = this.$check_action('/book_shortage_information/view','get');
